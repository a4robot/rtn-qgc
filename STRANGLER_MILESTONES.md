...
**ผลจริง wave 16:** **Q8b ✅** — portable QGCStateMachine (34 ไฟล์, replay-compare byte-identical ×5, จับ ordering bug จริงได้ 1 ตัว) → −StateMachine = **6 libs**; พบ blocker ใหม่ของ Gui: `QGCGeo.cc` ใช้ `QVector3D` (QtGui) → คิว wave 17 + **Q8d ✅** — §15 image channel (base64/JSON, MockLink ยิงภาพ test, byte-identical e2e, conformance 74→81) + **HttpServer cut ✅** + **Q8e/Q8f characterization baselines ✅** (UDP/TCP/serial-config/terrain/PBKDF2 — 5/5 เขียว; serial data-path ยังไม่มี pty tooling = ช่องว่างที่ Q8e ต้องปิด; พบ contract จริง: LinkManager ทำลาย link แบบ synchronous กลาง emission)
**ผลจริง wave 17: Q8g ✅** — audit ด้วย "linker enumerate survivors" (gate `Qt6::Gui` ออกจาก `QGCGeoMath` ก่อนแล้วดูว่า linker บ่นอะไรบ้าง) เจอผู้ใช้ Gui-type เกินกว่าที่ wave 16 คาด: `QGCGeo.{h,cc}` (`QVector3D`→`QGCGeo::Vec3`, double precision), `Joystick/*` (gyro/accel `QVector3D`→`QGCVector3D` ใหม่ที่ `Utilities/Math/`), `VehicleFactGroup.cc` (`QQuaternion`/`QVector3D` ในการหมุน attitude → local `AttitudeQuat` + cross-product formula เดิมของ Qt, verify ด้วย brute-force sandwich-product นอก repo), `Actuators/Common.h` (`ActuatorGeometry::position`), `QGCMapPolygon`+`SurveyComplexItem` (`QPolygonF`→`QList<QPointF>` + `QGC::polygonBoundingRect/polygonContainsPoint` มือใหม่ที่ `QGCMath`), `InstrumentValueData` (`QColor`→`QString`, QML รับ string coercion ได้), `QGCPalette`/`QGCMapPalette` (gate ทั้งไฟล์หลัง `QGC_ENABLE_QML`, ~40-entry theme table เป็น QML-rendering ล้วน ๆ), cursor feedback ใน `APMAirframeComponentController`/`AirframeComponentController` (gate `QCursor`/`QGuiApplication::*OverrideCursor`) — **และ DBus survivor ที่ไม่เคยรู้มาก่อน**: `qt6keychain` (vendored CPM) ต้องการ `Qt6::DBus` แบบ unconditional บน Linux (ไม่มี CMake option ปิดได้ — ยืนยันจาก upstream `CMakeLists.txt`) → gate ทั้ง package หลัง `QGC_ENABLE_QML`, `QGCKeychain.cc` ใช้ QSettings fallback ที่มีอยู่แล้วตรง ๆ เมื่อ headless. **ผล: ghost = 4 libs — Core, Network, Positioning, SerialPort** (−Gui, −DBus) ✅ conformance 69 PASS/0 FAIL/0 GAP, mockghostprobe 6/6, characterization ctest เขียวทั้งก่อน/หลัง (Geo/Joystick/FactGroup/QGCMapPolygon/APMAirframeComponentController)
**ผลจริง wave 17 (ต่อ): Q8e ✅** — SerialPort compat ใหม่ (termios+libudev in-house; ปฏิเสธ libserialport: LGPL-3.0 + autotools-only) หลัง flag `QGC_ENABLE_QT_SERIALPORT` ด้วย include-path shadowing (consumer ไม่ต้องแก้เลย); ครอบ SerialLink worker / QGCSerialPortInfo enumeration (libudev) / GPSProvider blocking mode / Bootloader flash; pty data-path test (posix_openpt ในเทสต์เอง) เขียวทั้ง 2 backends — เขียวกับ QSerialPort จริงก่อนตาม protocol; conformance 81/0/0 → **ghost = 3 libs: Core, Network, Positioning — เหลือ Q8f ชิ้นเดียวปิด M8**
ทุก job แยก directory กัน → ปล่อยขนานได้ตามสูตร swarm เดิม

**ผลจริง wave 18: Q8f ✅ — M8 จบ.** flag `QGC_ENABLE_QT_NETWORK` (default ON) แบ่ง QtNetwork เป็นสองชั้นตามผล audit จริง:
- **ชั้น socket/link (OFF-reachable, พอร์ตจริง):** `src/Comms/portable/QGCNetworkCompat.{h,cc}` — POSIX-socket compat in-house (IPv4-only) ของ `QHostAddress`/`QAbstractSocket`/`QUdpSocket`/`QTcpSocket`/`QTcpServer`/`QNetworkDatagram`/`QHostInfo`/`QNetworkInterface::allAddresses` ผ่าน include-path shadowing (`portable/QtNetwork/*`, สูตรเดียวกับ Q8e เป๊ะ — consumer ศูนย์ #ifdef สำหรับชั้นนี้) ครอบ UDPLink/TCPLink worker threads (QSocketNotifier readyRead), UdpIODevice/NMEA, ADSBTCPLink (buffered-read model ให้ canReadLine/readLine ทำงานเหมือน Qt), VideoManager Viewpro ping, APM Artoo ping, WebBridge listen-address; TCP มี async connect completion (write-notifier) + waitForConnected ทั้งคู่; connect-ล้มเหลว emit errorOccurred+state โดยไม่ emit disconnected (ตรง Qt — TCPLinkTest refused-connect pin ไว้); writeData retry-loop กัน 0-return ที่ TCPWorker ถือเป็น error
- **PBKDF2:** `QPasswordDigestor::deriveKeyPbkdf2` เขียนใหม่บน `QMessageAuthenticationCode` (QtCore — HMAC ไม่เคยอยู่ QtNetwork) **ไม่ใช้ OpenSSL เลย** (image ไม่มี openssl-dev; ไม่เพิ่ม dependency ใหม่ศูนย์ตัว) — ตรง pinned vector bit-for-bit + vector อิสระ iter=2/4096 multi-block (standalone check ใน docker คอมไพล์โค้ดจริงที่ extract จาก .cc)
- **ชั้น HTTP(S)/QNAM (ต้อง TLS จริงใน production):** ไม่ port — gate ด้วย #ifdef/CMake แล้ว degrade เป็น documented "unavailable in this build": Terrain network-fetch (cache-first ยังอยู่), QGCFileDownload/QGCCachedFileDownload + version check, NTRIP ทั้ง subdir (ใช้ QSslSocket), MAVLinkLogManager upload, VehicleCameraControl definition-XML http, ComponentInformation HTTP metadata (FTP path ไม่กระทบ), QGCCachedTileSet bulk download, `UrlFactory::getTileNetworkRequest` — **การตัดสินใจ TLS:** vendor TLS client หนักเกิน wave และ QNAM-over-Qt ยังเป็น ON-path ปกติ; นี่คือ capability note ของ diet ghost ไม่ใช่ GCS ปกติ; `QGC_ENABLE_QT_NETWORK=OFF` บังคับ `QGC_ENABLE_QML=OFF` (FATAL_ERROR — Qt6::Location hard-dep QtNetwork, ผสม header จริงกับ compat = ODR)
- **Verify:** ON backend ctest 13 suites before/after เท่ากันเป๊ะ (12/13 — `SigningTest::_testAddRawKey` fail เฉพาะ env dbus-keychain, pre-existing ที่ HEAD, Pbkdf2 vector PASS ทั้งก่อน/หลัง); OFF ghost: mockghostprobe 6/6, conformance 81/0/0, **UDP data-path e2e ใหม่** (MAVLink v2 heartbeat จริง sysid 66 → compat RX → vehicle โผล่ใน tick + return traffic กลับถึง peer = compat TX), PBKDF2 standalone 3/3 — **ldd สุดท้าย: libQt6Core + libQt6Positioning เท่านั้น = daemon target ของ Option A ถึงจริง**
