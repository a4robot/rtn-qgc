...
void StateMachineLogger::setEnabled(bool enabled)
{
    ...
    if (_enabled) {
        _connections.append(connect(_machine, &QGCStateMachine::started,
                                    this, &StateMachineLogger::_onMachineStarted));
        _connections.append(connect(_machine, &QGCStateMachine::stopped,
...
void StateMachineLogger::_connectToState(QGCAbstractState *state)
{
    auto entryConn = connect(state, &QGCAbstractState::entered,
                             this, &StateMachineLogger::_onStateEntered);
    auto exitConn = connect(state, &QGCAbstractState::exited,
