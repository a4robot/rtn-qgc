...
SendMavlinkMessageState::SendMavlinkMessageState(QGCStateMachine *machine, MessageEncoder encoder, int retryCount)
    : QGCState(QStringLiteral("SendMavlinkMessageState"), machine)
    , _encoder(std::move(encoder))
    , _retryCount(retryCount)
{
    connect(this, &QGCAbstractState::entered, this, &SendMavlinkMessageState::_sendMessage);
    // Note: _runCount is NOT reset on exit - it tracks retries across re-entries.
    // The state machine is expected to be one-shot (delete after finished).
}
