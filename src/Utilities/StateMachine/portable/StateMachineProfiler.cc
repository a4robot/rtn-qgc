...
StateMachineProfiler::StateMachineProfiler(QGCStateMachine *machine)
    ...
    , _machine(machine)
{
    connect(_machine, &QGCStateMachine::started, this, &StateMachineProfiler::_onMachineStarted);
    connect(_machine, &QGCStateMachine::stopped, this, &StateMachineProfiler::_onMachineStopped);
}
...
void StateMachineProfiler::setEnabled(bool enabled)
{
    ...
    if (_enabled) {
        const auto states = _machine->findChildren<QGCAbstractState *>();
        for (QGCAbstractState *state : states) {
            auto enterConn = connect(state, &QGCAbstractState::entered, this, &StateMachineProfiler::_onStateEntered);
            auto exitConn = connect(state, &QGCAbstractState::exited, this, &StateMachineProfiler::_onStateExited);
